...
GM60_scanner::GM60_scanner(Stream * serial) {
  mySerial = serial;
}


int *GM60_scanner::get_response()
{
  static int buf[20];
  int count = 0;
...
void GM60_scanner::clear_buffer()
{
...
void GM60_scanner::init()
{
  mySerial->write(set_default, 9);
  delay(10000);
  mySerial->write(set_serial_output, 9);
  delay(1000);
  //GM60_scanner::get_response();
  //GM60_scanner::clear_buffer();
}

void GM60_scanner::enable_setting_code()
{
...
void GM60_scanner::disable_setting_code()
{
...
int GM60_scanner::get_mode(byte addr1, byte addr2)
{
  read_reg[4] = addr1;
  read_reg[5] = addr2;
  GM60_scanner::clear_buffer();
  ...
  delay(1000);
  int *p;
  p = GM60_scanner::get_response();
  return *(p + 4);

}

void GM60_scanner::set_silent_mode(uint8_t silent_mode)
{
...
void GM60_scanner::set_LED_mode(uint8_t LED_mode)
{
  int current_mode = get_mode(0x00, 0x00);
...
void GM60_scanner::set_working_mode(uint8_t working_mode) //bit0-1
{
  int current_mode = get_mode(0x00, 0x00);
...
void GM60_scanner::set_light_mode(uint8_t light_mode) //bit2-3
{
  int current_mode = get_mode(0x00, 0x00);
...
void GM60_scanner::set_aim_mode(uint8_t aim_mode) //bit4-5
{
  int current_mode = get_mode(0x00, 0x00);
...
void GM60_scanner::scan_once()
{
    mySerial->write(scan_once_command, 9);
}

void GM60_scanner::set_sleep_mode(uint8_t sleep_mode){
